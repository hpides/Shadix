#!/usr/bin/env bash
# Tier 3: throughput end-to-end against a real nats-server.
#
# Usage: throughput_integration.sh <core|js> <sync|async> [TPUT_BIN]
set -u

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TRANSPORT="${1:?usage: throughput_integration.sh <core|js> <sync|async> [TPUT_BIN]}"
MODE="${2:?usage: throughput_integration.sh <core|js> <sync|async> [TPUT_BIN]}"
TPUT="${3:-${TPUT_BIN:-$SCRIPT_DIR/../build/throughput}}"
...
sleep 1  # let consumer subscriptions establish (core drops otherwise)
...
if [ "$TRANSPORT" = "js" ]; then
    # JetStream WorkQueue guarantees delivery: all items must arrive.
    [ "$received" -eq "$TOTAL" ] \
        || fail "js consumer received $received items, want exactly $TOTAL"
else
    # core NATS is at-most-once: require progress, not completeness.
    [ "$received" -gt 0 ] || fail "core consumer received nothing"
fi
echo "ok: consumer received $received items"

echo "throughput_integration ($TRANSPORT, $MODE): all checks passed"
