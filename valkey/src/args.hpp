...
// --send-mode, shared by rtt (send/server) and throughput (produce).
//   sync:  one broker round trip per item (RPUSH, wait for the reply)
//   async: pipelined RPUSH; replies are settled later (see ListProducer)
