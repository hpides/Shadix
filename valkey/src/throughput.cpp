// Throughput benchmark using C++ and libvalkey.
//
// Subcommands:
//   start-time <offset>   prints CLOCK_MONOTONIC_RAW + offset (synchronization)
//   init                  DEL key
//   produce               batches × batch_size RPUSH per task, with per-batch
//                         completion timestamps written to CSV
//   consume               BLPOP loop for `duration`
//
// Each worker owns one blocking libvalkey connection.
//
// --send-mode sync  (default): one broker round trip per item (RPUSH, wait
//                   for the reply).
// --send-mode async: RPUSH is pipelined — the batch's commands are queued
//                   (enqueue), written in one go and their replies read by
//                   the flush_batch() barrier at each batch boundary. The batch completion
//                   timestamp is taken after the barrier returns. A failed
//                   barrier is fatal: items may never have reached the
//                   broker, so no CSV or summary is produced and the process
//                   exits nonzero (same policy as the nats/pulsar harnesses).
...
Common parse_common(const std::vector<std::string>& args) {
    ...
    c.key = find_opt(args, "key").value_or("rb:tput");
    return c;
}
...
void cmd_init(const Common& c) {
    ...
    init_list_key(ctx.get(), c.key);
    std::cout << "Initialized benchmark key: key=" << c.key << '\n';
}
...
void cmd_produce(const Common& c, const std::vector<std::string>& args) {
    ...
    for (std::size_t t = 0; t < concurrency; ++t) {
        threads.emplace_back([&, t]() {
            try {
                auto ctx = make_connection(c.url);
                ListProducer producer(std::move(ctx), c.key);
                ...
                for (std::uint64_t bi = 0; bi < batches; ++bi) {
                        ...
                        while (true) {
                            try {
                                if (send_mode == SendMode::sync_mode) {
                                    // Broker round trip per item.
                                    producer.send(payload.data(),
                                                  payload.size());
                                } else {
                                    // Pipelined; settled at batch boundary.
                                    producer.enqueue(payload.data(),
                                                           payload.size());
                                }
                                break;
                            } catch (const ConnectionError&) {
                                // The context is dead for good (no
                                // reconnect): retrying would spin forever,
                                // so fail the task instead.
                                throw;
                    ...
                    if (send_mode == SendMode::async_mode) {
                        ...
                        // every pipelined RPUSH has been answered. A failed
                        // barrier means items may never have arrived, so it
                        // is fatal: the run's results would misreport
                        // delivered throughput (same policy as nats/pulsar).
...
void cmd_consume(const Common& c, const std::vector<std::string>& args) {
    ...
    for (std::size_t t = 0; t < concurrency; ++t) {
        threads.emplace_back([&, t]() {
            auto ctx = make_connection(c.url);
            ListConsumer consumer(std::move(ctx), c.key);
...
void print_usage() {
    std::cerr <<
        ...
        "Subcommands:\n"
        "  start-time OFFSET\n"
        "  init     --url URL --transport list -s SIZE [--key K]\n"
